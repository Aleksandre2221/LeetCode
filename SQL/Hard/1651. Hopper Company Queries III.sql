...
SELECT 
    EXTRACT(MONTH FROM m.month) "month",
    ROUND(COALESCE(SUM(a.ride_distance), 0)::numeric / 3, 2) average_ride_distance,
    ROUND(COALESCE(SUM(a.ride_duration), 0)::numeric / 3, 2) average_ride_duration
FROM months m
LEFT JOIN rides r
...
GROUP BY EXTRACT(MONTH FROM m.month)
ORDER BY "month";
